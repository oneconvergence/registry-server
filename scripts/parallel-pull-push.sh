...
echo Done pulling and pushing images from: $list | tee -a $log_file $metrics_file > /dev/null
if [[ $list == *-ds.txt ]]; then
	echo Finished uploading all images! | tee -a $log_file $metrics_file > /dev/null
fi
echo -------------------------------------------------------- >> $metrics_file
